...
- モードBで、lastmodが変化した子urlsetの実URL数が1チェックの処理上限（`MAX_FEED_ITEMS_PER_CHECK`）を超える場合、超過分は当該チェックでは処理せず、次回以降のチェックに持ち越す。この場合ベースライン／新着検知はその分だけ**部分的・遅延的**になる（初回に「全件を1回で確定」はしない）。打ち切り件数はログとメタに記録し、無言で落とさない。lastmod足切り（3日）により、実際に処理対象となる実URLは通常この上限より十分小さくなる想定。
